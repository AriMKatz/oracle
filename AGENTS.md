...
- Fork maintenance: before incorporating changes from `steipete/oracle`, read
  [UPSTREAM_SYNC.md](UPSTREAM_SYNC.md). Keep its synchronization procedures,
  classifications, and maintenance rationale out of `skills/oracle/SKILL.md`
  and Oracle prompts. Preserve executor-side requirements that
  `UPSTREAM_SYNC.md` explicitly designates for the installed skill, including
  standing informed external-upload consent; never inject those requirements
  into Oracle prompts or attachments.
...
- Pro browser runs: allow up to 10 minutes for ordinary live smokes and up to 120 minutes for large full-context reviews; never click "Answer now"; keep at least 1–2 Pro live tests (reattach must stay Pro); move other tests to faster models where safe.
...
- Browser “Pro thinking” gate: never click/auto-click ChatGPT’s “Answer now” button. Treat it as a placeholder and wait up to the configured browser deadline (as long as 120 minutes for large full-context reviews) for the real assistant response; auto-clicking skips long thinking and changes behavior.

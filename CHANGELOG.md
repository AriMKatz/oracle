...
- Browser: use the same complete `/c/` path-segment parser in attachment verification, chat-mode probes, and assistant-response scoping; retain a transient post-submit `WEB:*` identity while allowing only its one-way same-target promotion to ChatGPT's canonical conversation ID; and distinguish empty CDP evaluations from positively observed conversation changes during both user-turn binding and report monitoring, with exact diagnostics.

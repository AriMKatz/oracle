...
- Browser artifacts: browser sessions save `transcript.md` and generated artifacts under `~/.oracle/sessions/<id>/artifacts/`. Deep Research saves `deep-research-report.md` when the report surface is captured. Each detected interactive numbered citation is linked only when its index has one unambiguous primary HTTP(S) URL; unresolved citations stay unlinked and produce a warning. A clean zero-citation status requires affirmative zero-citation UI evidence, so an empty selector scan cannot hide UI-schema drift. When available, Deep Research records the selected/default Pro model separately from ChatGPT's exact report-owner/orchestration and resolved model slugs, which may differ; missing metadata produces a warning, and each field retains only its recorded role. ChatGPT-generated images and downloadable files are saved with the active browser session when supported file URLs are present.
